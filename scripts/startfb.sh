...
nohup ${fbpath} -E MON_ADDR=${local_ip} -E FILEPATH=${db_logpath} -e -c ${fbconf} -d publish -path.home /usr/share/filebeat -path.config /etc/filebeat -path.data /var/lib/filebeat -path.logs /var/log/filebeat &
